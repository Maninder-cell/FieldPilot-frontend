  ...
  return (
    <div className="w-full">
      <form onSubmit={handleSubmit} className="space-y-6 sm:space-y-8">
        ...
        <div className="flex justify-center">
          <div className="relative inline-flex items-center bg-gray-100 rounded-full p-1 shadow-inner">
            <button
              ...
              className={`px-6 sm:px-8 py-2 sm:py-3 rounded-full font-semibold text-sm sm:text-base transition-all duration-300 ${billingCycle === 'monthly'
            ...
            <button
              ...
              className={`px-6 sm:px-8 py-2 sm:py-3 rounded-full font-semibold text-sm sm:text-base transition-all duration-300 ${billingCycle === 'yearly'
                ? 'bg-white text-gray-900 shadow-md'
                : 'text-gray-600 hover:text-gray-900'
                }`}
            >
          ...
          {plans.map((plan, index) => {
            ...
            return (
              <div
                ...
                className={`relative bg-white rounded-2xl border-2 p-4 sm:p-6 md:p-8 flex flex-col h-full shadow-sm cursor-pointer group ${isRecommended
              ...
              >
                ...
                {/* Plan Name */}
                <h3 className="text-xl sm:text-2xl md:text-3xl font-extrabold text-gray-900 mb-2 sm:mb-3">{plan.name}</h3>

                {/* Plan Description */}
                <p className="text-gray-600 text-xs sm:text-sm md:text-base mb-3 sm:mb-4 md:mb-6 min-h-[2rem] sm:min-h-[2.5rem] md:min-h-12">{plan.description}</p>

                {/* Pricing */}
                <div className="mb-4 sm:mb-6 md:mb-8 pb-3 sm:pb-4 md:pb-6 border-b border-gray-200">
                  <div className="flex items-baseline flex-wrap gap-1">
                    <span className="text-3xl sm:text-4xl md:text-5xl font-extrabold text-gray-900">${price}</span>
                    <span className="text-gray-500 text-sm sm:text-base md:text-lg whitespace-nowrap">
                ...
                {/* Resource Limits */}
                <div className="mb-3 sm:mb-4 md:mb-6 space-y-2 sm:space-y-3 bg-gray-50 rounded-lg p-2 sm:p-3 md:p-4">
                  <div className="flex items-center text-xs sm:text-sm">
                    <div className="w-6 h-6 sm:w-7 sm:h-7 md:w-8 md:h-8 rounded-full bg-teal-100 flex items-center justify-center mr-2 sm:mr-3 shrink-0">
                      <svg className="w-3 h-3 sm:w-3.5 sm:h-3.5 md:w-4 md:h-4 text-teal-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                {displayFeatures.length > 0 && (
                  <div className="mb-4 sm:mb-6 md:mb-8 flex-1 flex flex-col">
                    {previousPlanText ? (
                      <div className="mb-3 sm:mb-4 md:mb-5">
                        <div className="inline-flex items-center gap-2 bg-gradient-to-r from-teal-50 to-blue-50 px-2 sm:px-3 py-1.5 sm:py-2 rounded-lg border border-teal-200">
                          <svg className="w-3 h-3 sm:w-4 sm:h-4 text-teal-600 shrink-0" fill="currentColor" viewBox="0 0 20 20">
                      ...
                      <h4 className="text-xs sm:text-sm font-bold text-gray-900 mb-2 sm:mb-3 md:mb-4 uppercase tracking-wide">
                        What's Included:
                      </h4>
                    )}
                    <ul className="space-y-1.5 sm:space-y-2 md:space-y-3 flex-1">
                      {displayFeatures.map((feature, idx) => (
                        <li key={idx} className="flex items-start text-xs sm:text-sm">
                          <svg className="w-3.5 h-3.5 sm:w-4 sm:h-4 md:w-5 md:h-5 text-teal-500 mr-2 sm:mr-3 shrink-0 mt-0.5" fill="currentColor" viewBox="0 0 20 20">
        ...
        {/* Trial Notice */}
        <div className="bg-gradient-to-r from-teal-50 to-blue-50 rounded-xl p-3 sm:p-4 md:p-6 border border-teal-100">
